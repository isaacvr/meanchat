...
myChat.controller('mainController', [
  ...
  function($scope, $window, io) {
    ...
    $scope.cloneObject = function cloneObject(o) {

      var k = Object.keys(o);

      if (k.length === 0 || typeof o === 'string') {
        return o;
      }

      var res = o.constructor.call(null);

      k.forEach(function(a, b, c) {
        res[a] = cloneObject(o[a]);
      });
    ...
    $scope.selectRoom = function selectRoom(r) {
      ...
      if (!$scope.rooms.hasOwnProperty(r)) {
        $scope.rooms[ r ] = $scope.cloneObject( $scope.EMPTY_ROOM );
      } else {
        $scope.rooms[ r ].unreaded = 0;
      }
    ...
    $scope.addMessage = function addMessage(msg, from, to, avatar, type, color, fcolor) {
      ...
      if (userRoom === $scope.myId && !fcolor === true) {
        ...
        if (!$scope.rooms.hasOwnProperty(userRoom)) {
          $scope.rooms[ userRoom ] = $scope.cloneObject( $scope.EMPTY_ROOM );
        }

        if (userRoom != $scope.userSelected) {
          $scope.userlist[ userRoom ].class = 'new-message';
          $scope.rooms[ userRoom ].unreaded = $scope.rooms[ userRoom ].unreaded || 0;
          $scope.rooms[ userRoom ].unreaded += 1;
        }
      }
      ...
      $scope.rooms[ userRoom ].messages.push(tab);
      //$scope.rooms[ userRoom ].unreaded = $scope.rooms[ userRoom ].unreaded || 0;
      //$scope.rooms[ userRoom ].unreaded += 1;
    ...
    $scope.socket.on('USER_DATA', function(data) {
      ...
      $scope.$apply(function() {
        ...
        if (!$scope.rooms[ data.id ]) {
          $scope.rooms[ data.id ] = $scope.cloneObject( $scope.EMPTY_ROOM );
        }
